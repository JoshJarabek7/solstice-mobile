...
struct DatingSettingsView: View {
  ...
  private var photosSection: some View {
    ScrollView {
      ...
      LazyVGrid(columns: columns, spacing: 12) {
        ForEach(Array(userViewModel.user.datingImages.enumerated()), id: \.offset) {
          index, photoURL in
          GeometryReader { geometry in
            AsyncImage(url: URL(string: photoURL)) { phase in
              if let image = phase.image {
                  ...
                  image
                    ...
                    .overlay(
                      RoundedRectangle(cornerRadius: 12)
                        .stroke(
                          selectedPhotoIndex == index ? Color.blue : Color.gray.opacity(0.3),
                ...
                .confirmationDialog(
                  "Photo Options", isPresented: $showPhotoOptions, presenting: selectedPhotoIndex
                ) { index in
                  if index > 0 {
                    Button("Move Left") {
  ...
  private func movePhoto(from: Int, to: Int) {
    guard from != to,
      ...
      to < userViewModel.user.datingImages.count
    else { return }
