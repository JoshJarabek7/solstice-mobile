...
final class VideoCreationViewModel: ObservableObject {
  ...
  private func detectVideoAspectRatio(from asset: AVAsset) async throws -> VideoAspectRatio {
    ...
    guard let track = tracks.first else {
      throw NSError(
        domain: "", code: -1, userInfo: [NSLocalizedDescriptionKey: "No video track found"])
    }
    ...
    Task {
      do {
        guard let videoData = try await selectedItem.loadTransferable(type: Data.self) else {
          throw NSError(
            domain: "", code: -1, userInfo: [NSLocalizedDescriptionKey: "Failed to load video data"]
          )
        }
  ...
  func uploadVideo() async throws {
    guard let videoURL = selectedVideoURL else {
      throw NSError(
        domain: "", code: -1, userInfo: [NSLocalizedDescriptionKey: "No video selected"])
    }
    ...
    guard FileManager.default.fileExists(atPath: videoURL.path) else {
      throw NSError(
        domain: "", code: -1,
        userInfo: [NSLocalizedDescriptionKey: "Video file not found at path: \(videoURL.path)"])
    }
    ...
    do {
      ...
      try FileManager.default.createDirectory(
        at: processedDirectory, withIntermediateDirectories: true)
      ...
      guard FileManager.default.isReadableFile(atPath: finalVideoURL.path) else {
        throw NSError(
          domain: "", code: -1,
          userInfo: [NSLocalizedDescriptionKey: "Final processed video is not readable"])
      }

      // Get file size for logging
      let fileSize =
        try FileManager.default.attributesOfItem(atPath: finalVideoURL.path)[.size] as? Int64 ?? 0
      ...
      guard fileSize > 0 else {
        throw NSError(
          domain: "", code: -1,
          userInfo: [NSLocalizedDescriptionKey: "Processed video file is empty"])
      }
  ...
  private func trimVideoIfNeeded(_ videoURL: URL) async throws -> URL {
    ...
    throw NSError(
      domain: "", code: -1,
      userInfo: [NSLocalizedDescriptionKey: "Export failed: Could not create export session"])
  }
